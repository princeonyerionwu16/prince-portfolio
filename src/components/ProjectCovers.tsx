...
import {
  Search, Sparkles, Star, BookOpen, Bell, Plus,
  CheckCircle2, BarChart3, Filter, Calendar,
  Layers, Users, GraduationCap, MoreHorizontal,
  Play, ArrowRight,
} from "lucide-react";
...
---------------------------------------------------------- */
function BrowserChrome({ url, children, theme = "dark" }: { url: string; children: React.ReactNode; theme?: "dark" | "light" }) {
...
export function LumenReadsCover() {
  ...
  return (
    <BrowserChrome url="lumenreads.app/discover">
      {/* subtle aurora */}
      <div className="absolute inset-0" style={{ background: "radial-gradient(at 15% 0%, oklch(0.45 0.18 295 / 0.30), transparent 55%), radial-gradient(at 100% 100%, oklch(0.50 0.18 255 / 0.25), transparent 55%)" }} />
      ...
      <div className="relative flex h-full">
        ...
        <aside className="hidden w-[22%] shrink-0 border-r border-white/5 p-2.5 sm:block">
          ...
          <div className="mt-3 space-y-1">
            {["Discover", "Library", "Lists", "Friends"].map((l, i) => (
              <div key={l} className={`rounded-md px-1.5 py-1 font-mono text-[8px] ${i === 0 ? "bg-white/10 text-white" : "text-white/45"}`}>{l}</div>
            ))}
          </div>
        </aside>
        ...
        <div className="flex-1 p-3">
          ...
          >
            ...
            <span className="font-mono text-[9px] text-white/70">books like "Dune" but cozier…</span>
            <motion.span className="ml-auto h-3 w-px bg-fuchsia-300/80" animate={{ opacity: [1, 0, 1] }} transition={{ duration: 1.1, repeat: Infinity }} />
          </motion.div>
          ...
          <div className="mt-2.5 grid grid-cols-3 gap-1.5">
            {books.map((b, i) => (
              ...
              >
                ...
                <p className="mt-1 truncate font-display text-[9px] font-semibold text-white">{b.t}</p>
...
export function TaskFlowCover() {
  const cols = [
    { t: "Backlog", n: 4, c: "bg-sky-400", tasks: [{ p: "high", l: "Onboarding flow" }, { p: "med", l: "Refactor auth" }] },
    { t: "In progress", n: 2, c: "bg-amber-400", tasks: [{ p: "high", l: "Billing portal" }] },
    { t: "Done", n: 6, c: "bg-emerald-400", tasks: [{ p: "low", l: "Landing v2" }, { p: "med", l: "Migrate DB" }] },
  ];
  const pColor: Record<string, string> = { high: "bg-rose-400/80", med: "bg-amber-400/80", low: "bg-emerald-400/80" };

  return (
    <BrowserChrome url="taskflow.pro/board/sprint-12">
      <div className="absolute inset-0" style={{ background: "radial-gradient(at 80% 0%, oklch(0.55 0.15 200 / 0.25), transparent 60%)" }} />
      ...
      <div className="relative flex h-full flex-col">
        ...
        <div className="flex h-7 items-center gap-2 border-b border-white/10 px-3">
          ...
          <span className="rounded-full bg-cyan-500/15 px-1.5 py-0.5 font-mono text-[7px] text-cyan-300">active</span>
          ...
          {cols.map((c, ci) => (
            <div key={c.t} className="rounded-md border border-white/10 bg-white/[0.025] p-1.5">
              <div className="flex items-center gap-1">
                ...
                <span className="font-mono text-[7.5px] uppercase tracking-wider text-white/65">{c.t}</span>
                ...
                {c.tasks.map((tk, i) => (
                  ...
                  >
                    ...
                    <div className="mt-1 flex items-center gap-1">
                      <span className="rounded bg-white/10 px-1 font-mono text-[6.5px] text-white/55">TF-{20 + i}</span>
          ...
          <div className="rounded-md border border-white/10 bg-white/[0.03] p-1.5">
            ...
            <div className="mt-1 flex items-end gap-0.5">
              {[3, 5, 4, 7, 6, 9, 8].map((h, i) => (
                <motion.span
                  key={i}
                  initial={{ height: 0 }} whileInView={{ height: `${h * 2}px` }} viewport={{ once: true }}
...
export function AuraStudioCover() {
  return (
    <BrowserChrome url="aura.studio">
      {/* gradient field */}
      <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, oklch(0.17 0.05 320), oklch(0.15 0.04 270), oklch(0.16 0.05 230))" }} />
      <motion.div
        className="absolute -left-8 -top-8 h-40 w-40 rounded-full"
        style={{ background: "radial-gradient(circle, oklch(0.7 0.22 320 / 0.40), transparent 70%)" }}
        animate={{ x: [0, 20, 0], y: [0, 15, 0] }}
        transition={{ duration: 11, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute -right-8 bottom-0 h-44 w-44 rounded-full"
        style={{ background: "radial-gradient(circle, oklch(0.7 0.22 230 / 0.40), transparent 70%)" }}
      ...
      <div className="relative flex h-7 items-center gap-3 border-b border-white/8 px-3">
        ...
        <button className="ml-auto rounded-full bg-white px-2 py-0.5 font-mono text-[7.5px] font-semibold text-black">contact</button>
      </div>
      ...
      <div className="relative flex h-[calc(100%-1.75rem)] flex-col items-start justify-center px-5">
        ...
        >
          We craft <span className="bg-gradient-to-r from-fuchsia-300 via-violet-300 to-cyan-300 bg-clip-text text-transparent">light</span><br />into pixels.
        </motion.h3>
        ...
        <aside className="hidden w-[18%] shrink-0 border-r border-white/8 p-2 sm:block">
          ...
          <div className="mt-3 space-y-1">
            {[
              ...
              <div key={l} className={`flex items-center gap-1.5 rounded px-1.5 py-1 ${a ? "bg-emerald-500/10 text-emerald-300" : "text-white/40"}`}>
        ...
        <div className="flex-1 p-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-1.5">
              ...
              <button className="rounded-md bg-emerald-400 px-2 py-1 font-mono text-[7.5px] font-semibold text-black">+ add</button>
            </div>
          </div>
          ...
          <div className="mt-2 rounded-md border border-white/10 bg-black/20 p-1.5">
            <div className="grid grid-cols-[1.4fr_60px_50px_30px_16px] items-center gap-2 border-b border-white/10 pb-1">
              {["Student", "Course", "Status", "Grade", ""].map((h) => (
                <span key={h} className="font-mono text-[7px] uppercase text-white/40">{h}</span>
              ))}
            </div>
            {students.map((r, i) => (
              <motion.div
              ...
              >
                ...
                <span className={`inline-flex items-center gap-0.5 rounded-full px-1 py-0.5 font-mono text-[7px] ${r.s === "active" ? "bg-emerald-500/15 text-emerald-300" : "bg-amber-500/15 text-amber-300"}`}>
